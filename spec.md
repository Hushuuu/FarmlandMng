...
一般操作維持上述軟刪除策略；若需清理測試資料或誤建資料，可在系統設定輸入管理密碼解鎖「永久刪除」。永久刪除會一併清理相關排程、執行批次與逐樹明細，且無法復原，正式環境應改由伺服器端權限控管。
...
一般情況下歷史紀錄仍然必須存在；只有管理模式明確執行永久刪除時才會一併移除。
...
使用者可在任務尚未開始時調整下一輪預計開始日；若該任務已有進行中的
Execution Batch，調整日期時則更新該批次的 `scheduled_date`，只影響本輪，
不會改變下一輪排程。
